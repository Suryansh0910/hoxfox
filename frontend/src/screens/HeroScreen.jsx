...
export const HeroScreen = ({ onNext }) => {
  ...
  return (
    <div className="screen flex-row h-screen overflow-hidden relative">
      <HoxNav mounted={mounted} onNext={onNext} />

      <AlbumLayer mounted={mounted} />

      <div className="absolute inset-0 z-[2] pointer-events-none" style={{ opacity: mounted ? 1 : 0, transition: 'opacity 1.2s 0.1s' }}>
        <WaveCanvas />
      </div>

      <div className="absolute left-0 top-0 w-full h-full z-[3] pointer-events-none"
        style={{ opacity: mounted ? 1 : 0, transition: 'opacity 0.8s 0.4s' }}>
        {[
          { label: 'Hip-Hop', color: 'rgba(120,60,255,0.85)', top: '20%' },
          { label: 'Rock', color: 'rgba(255,100,60,0.85)', top: '32%' },
          { label: 'Electronic', color: 'rgba(60,180,255,0.85)', top: '50%' },
          { label: 'Jazz', color: 'rgba(60,220,160,0.85)', top: '63%' },
          { label: 'Pop', color: 'rgba(255,200,60,0.85)', top: '75%' },
        ].map(g => (
      ...
      <div className="absolute right-8 top-1/2 -translate-y-1/2 z-[3] text-right pointer-events-none"
        style={{ opacity: mounted ? 1 : 0, transition: 'opacity 0.8s 0.6s' }}>
      ...
      <div className="absolute inset-0 z-[4] flex items-center justify-center pt-14 pointer-events-none">
        <div className="w-[440px] pointer-events-auto bg-[oklch(0.08_0.025_275)] border border-[oklch(0.20_0.04_275)] rounded-[24px] overflow-hidden shadow-[0_32px_80px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.05)]"
          style={{
            opacity: mounted ? 1 : 0,
          ...
          <div className="px-6 pb-6 flex flex-col gap-3">
            <button className="btn-primary w-full justify-center text-[15px] py-3.5 px-6" onClick={onNext}>
            ...
            <button className="btn-ghost w-full justify-center text-[14px] py-[11px] px-6" onClick={onNext}>
              Try with public playlist →
            </button>
          </div>
        </div>
      </div>
